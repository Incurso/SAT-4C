...
export default class AXL {
  async execute (method, type, content) {
    ...
    console.log(res.status, methodType)
    ...
    if (soapenvFault) {
      ...
      const response = nsResponse.return[type.charAt(0).toLowerCase() + type.slice(1)] || []
  ...
  apply (type, uuid) {
    return this.execute('apply', type, `<uuid>${uuid}</uuid>`)
  }
  ...
  get (type, searchCriteria) {
    ...
    return this.execute('get', type, content)
  }
  ...
  list (type, searchCriteria, returnedTags) {
    ...
    return this.execute('list', type, content)
  }
  ...
  updateUserDeviceAssociation (userid, devices) {
    ...
    return this.execute('update', 'User', content)
  }
  ...
  updateLine ({ pattern, callForwardAll }) {
    ...
    return this.execute('update', 'Line', content)
  }
